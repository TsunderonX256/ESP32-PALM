...
static void initScaleMaps() {
  ...
  for (int y = 0; y < PALM_LCD_VIEW_H; ++y) {
    palmLcdViewToSourceX[y] = (static_cast<uint32_t>(y) * ((PALM_LCD_W - 1) << 16)) / (PALM_LCD_VIEW_H - 1);
  }
  for (int x = 0; x < PALM_LCD_VIEW_W; ++x) {
    uint32_t forwardY16 = (static_cast<uint32_t>(x) * ((PALM_LCD_H - 1) << 16)) / (PALM_LCD_VIEW_W - 1);
    palmLcdViewToSourceY[x] = ((PALM_LCD_H - 1) << 16) - forwardY16;
  }
}

static uint16_t bilinear565(const uint16_t *pixels, int width, int height, uint32_t x16, uint32_t y16) {
  uint32_t x0 = x16 >> 16;
  uint32_t y0 = y16 >> 16;
  if (x0 >= static_cast<uint32_t>(width)) x0 = width - 1;
  if (y0 >= static_cast<uint32_t>(height)) y0 = height - 1;
  uint32_t x1 = (x0 + 1 < static_cast<uint32_t>(width)) ? x0 + 1 : x0;
  uint32_t y1 = (y0 + 1 < static_cast<uint32_t>(height)) ? y0 + 1 : y0;
  ...
  uint16_t c00 = pixels[y0 * width + x0];
  uint16_t c10 = pixels[y0 * width + x1];
  uint16_t c01 = pixels[y1 * width + x0];
  uint16_t c11 = pixels[y1 * width + x1];
...
static void renderPanelLcdFrameFromSurface(uint16_t *target) {
  ...
  for (int dy = 0; dy < PALM_LCD_VIEW_H; ++dy) {
    ...
    for (int dx = 0; dx < PALM_LCD_VIEW_W; ++dx) {
      lcdRow[dx] = palmSurface[static_cast<uint32_t>(palmLcdViewToSourceY[dx] >> 16) * PALM_SURFACE_W + srcX];
    }
  }
}
...
static void renderVirtualButtonStrip(uint16_t *target) {
  ...
  panelFillRect(target, VIRTUAL_BUTTON_STRIP_X0, 0, stripW, SCREEN_H, TFT_DARKGREY);
  panelDrawRect(target, VIRTUAL_BUTTON_STRIP_X0, 0, stripW, SCREEN_H, TFT_BLACK);
  ...
  for (int i = 0; i < VIRTUAL_BUTTON_COUNT; ++i) {
    ...
    uint16_t fill = (i == 2 || i == 3) ? 0x4a49 : 0x39e7;
    panelFillRect(target, bx, by, bw, bh, fill);
    panelDrawRect(target, bx, by, bw, bh, TFT_BLACK);

    if (i == 2) {
      panelDrawArrowIcon(target, bx, by, bw, bh, true);
    } else if (i == 3) {
      panelDrawArrowIcon(target, bx, by, bw, bh, false);
    } else {
      int bars = i < 2 ? i + 1 : i - 1;
      panelDrawAppIcon(target, bx, by, bw, bh, bars);
    }
  }
}
...
static void renderVirtualPowerStrip(uint16_t *target) {
  ...
  panelFillRect(target, VIRTUAL_POWER_STRIP_X0, 0, stripW, SCREEN_H, TFT_DARKGREY);
  panelDrawRect(target, VIRTUAL_POWER_STRIP_X0, 0, stripW, SCREEN_H, TFT_BLACK);
  ...
  panelFillRect(target, bx, powerBy, bw, powerBh, 0x4a49);
  panelDrawRect(target, bx, powerBy, bw, powerBh, TFT_BLACK);
  panelDrawPowerIcon(target, bx, powerBy, bw, powerBh);
  ...
  panelFillRect(target, bx, saveBy, bw, saveBh, 0x4a49);
  panelDrawRect(target, bx, saveBy, bw, saveBh, TFT_BLACK);
  panelDrawSaveIcon(target, bx, saveBy, bw, saveBh);
  ...
  panelFillRect(target, bx, resetBy, bw, resetBh, 0x4a49);
  panelDrawRect(target, bx, resetBy, bw, resetBh, TFT_BLACK);
  panelDrawResetIcon(target, bx, resetBy, bw, resetBh);
}
...
static void publishPanelFrame() {
  if (!panelStaticFramesReady) {
    ...
    renderPanelStaticFrameFromSurface(panelFrames[1]);
    activePanelFrame = panelFrames[0];
...
static void printWakeDebugLine(const char *tag, uint16_t touchAdcX, uint16_t touchAdcY) {
  ...
  Serial.printf("DBG %s t=%lu pc=%08lx sp=%08lx sr=%04lx sleep=%u wake=%u irq=%u slices=%lu(+%lu) "
                "pll=%04x im=%04x/%04x st=%04x/%04x pend=%04x/%04x pd=%02x/%02x pol=%02x req=%02x kbd=%02x edge=%02x "
